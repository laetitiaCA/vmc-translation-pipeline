...
def main():
    ...
    file_names = [f for f in listdir('./csv_files') if isfile(join('./csv_files', f))]
    
    flows_names = [line.strip('\n') for line in file_names]
    flows_names = [line[:-4] for line in flows_names] # remove .csv from string
    ...
    with open('./folders_IDs_spreadsheets.json', 'w') as outfile:
    ...
    for fl in range(len(flows_names)):
        ...
        csv_file_path = './csv_files/' + file_names[fl]
